...
folder are mapped to the application's root namespace, 'Cardinalkeeper'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "Cardinalkeeper"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'Cardinalkeeper' in the same way
as `"Cardinalkeeper/sass/src"`.
